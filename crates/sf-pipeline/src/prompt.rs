...
pub fn build_prompt(
    ...
    // 尾部要强调两件事,前缀里提一次不够。
    //
    // ① **场景贴合度**。只写「场景:问路;等级 L1;生成 20 句」时,模型写着
    //    写着就跑题 —— 「问路」场景里出现 `My name is Tom.`、`This is a book.`,
    //    一个场景 60 句里只有 5 句真在问路。跑题的句子照样入库,库里的场景
    //    分组就成了摆设。
    //
    // ② **用足本级的空间**。L2 的头 6 批实测入库率只有 49%,而丢弃的
    //    61 句里 60 句是「与已有句子重复」—— 模型拿 L2 的规格写出了 L1
    //    水平的句子,撞上已经产好的 L1 内容。规格(句长上限、词表带)在前缀
    //    里给了,但没说"要用足",模型就只挑最保险的写法。
    let mut user = format!(
        "场景:{scene};等级 {};生成 {count} 句。\n\
         **每一句都必须是在「{scene}」这个场景里真会说出口的话**;\n\
         想不出更多本场景的说法就少写几句,不要拿通用寒暄句凑数。\n\
         **要用足这一级的空间**:句长可以接近 {} 词上限,该用的新词就用,\n\
         别写成更低等级也写得出来的句子(那些已经有了,会被判重复)。",
        spec.id, spec.max_words
    );
...
"#,
    ...
    fn tail_carries_scene_count_and_real_examples() {
        ...
        assert!(
            p.user.contains("用足这一级的空间"),
            "尾部要要求用足本级空间 —— 否则模型写出低一级也能写的句子,撞重复"
        );
        assert!(p.user.contains("12 词"), "要把本级句长上限告诉模型");
