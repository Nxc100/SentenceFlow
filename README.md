...
- **打字 / 拆句重组 / 听打 / 默写**四种练习模式,Leitner 五盒间隔复习(SRS)自动排程(默写在高级/精通解锁,需对应等级的句库);
- 答对的瞬间触发**签名时刻**——下划线消融、单词按句子成分聚拢成彩色卡片、词性胶囊弹入、音标浮现;颜色即教学信息,不是装饰;
- 六级课程规格(L1–L6,锚定 CEFR)与全标注句库格式已就绪,**零配置、完全离线可练**;出厂句库当前覆盖入门–中级(50 句)+ 8 个情景对话包,更高等级的题库仍在补 ——在此之前可用**生成工坊**为任意等级现场造句;
...
- **AI 萌宠**(默认关闭,可在导航里开启):拖一张图 —— 自家猫猫狗狗的照片、随手画的角色都行 —— 60 秒变成活在桌面上的宠物。抠图、做动画、骨骼绑定**全在本机**,零生成式云端、零 API Key;想让它学会走路,复制一段「咒语」去豆包/即梦生成一张网格图或一段视频拖回来即可。它还替你管提醒与番茄钟:到点抱着闹钟蹦过来;
...
crates/
  ...
  sf-pet/        AI 萌宠的纯逻辑内核(零 tauri、不依赖任何句流 crate):抠背景/切帧/
                 语义抠图(u2netp + tract)、骨骼烘焙(轮廓→网格→ARAP→步态→软光栅)、
                 图集合成与校验、出生视频、咒语包、宠物库落盘
  sf-wasm/       sf-core 的 wasm-bindgen 绑定(JSON 字符串 ABI)
apps/
  desktop/       Tauri 2 桌面端(产品主体):src-tauri Rust 壳(119 个 command)
                 + React 前端(今日/句库/情景对话/AI 造句/AI 聊天/AI 萌宠/报告/水平/设置)
                 + 透明桌宠窗(零框架 Canvas,vite 第二入口 pet.html)
