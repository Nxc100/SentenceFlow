...
| 权限 | `--auto` 自动批准(官方标注 dangerous);默认非交互下工具权限行为**待 M5 前置 spike 验证** |
...
- 纠错卡带「🔊 朗读」与「⭐ 收藏该句」(收藏进已有 favorites,后续可在
  内容库重练——学习闭环)。
...
- **权限**:默认不带 `--auto`(遇到需批准的工具按 opencode 默认策略);
  设置内「自动批准工具操作」开关,开启时红字警告
  「AI 将可以在工作目录内读写文件、执行命令,请只对信任的任务开启」;
- 模型沿用设置里选定的模型;停止按钮 = 杀子进程(保留已收内容);
