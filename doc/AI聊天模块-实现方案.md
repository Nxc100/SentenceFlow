...
小白看不懂 AI 的英文回复就聊不下去。做法与纠错卡同源,**不额外发一轮请求**:

- 输入框旁第二个开关「中文对照」(默认开,与「帮我纠错」并列,记在
  localStorage);
- 开启时 system 追加 `⟦zh⟧` 协议段,AI 在正文后单独一行给出**整段回复**
  的自然中文(不是逐词直译);同一次请求带回,零额外延迟与费用;
- 渲染:中文排在英文下方,虚线分隔、字号更小、颜色更浅 ——
  **英文始终是主角**,中文只作兜底;随消息存 `chat_message.zh`(幂等加列),
  回头翻记录仍在;
- 解析器由 `split_fix` 升级为 `split_annotations`:两个标记顺序不限,
  容错原则不变 —— 只有确实解析出标注才切正文,模型没按协议输出就整段
  保留,**绝不把回复截没**。

**真机修掉的连带 bug**:opencode 通道续聊(`-s`)时只发用户消息、system
仅在建会话那次送达,所以中途切换开关对老会话**不生效**(实测关掉「中文
对照」后仍继续输出中文;「帮我纠错」同理,只是此前没测到)。
修法:`ChatRequest` 新增 `per_turn` 字段 —— 每轮都要重申的输出契约。
OpenAI 兼容通道忽略它(system 本来每轮都发),opencode 通道在续聊消息前
拼上它。关掉开关时的措辞也从「省略规则」改成**显式禁止**输出该标记。
...
### 3.5b opencode 技能面板(v1.2 追加)

把 opencode TUI 的技能选择器搬进图形界面,并补上 TUI 给不了的部分。

**数据源:`opencode debug skill`**(官方子命令,输出 JSON:
`name`/`description`/`location`/`content`)。去重、根目录优先级、内置技能
全部按 opencode 自己的解析结果来 —— **不自己实现一套规则**。真机依据:
同一台机器三处全局根目录装了大量重名技能(`.config/opencode` 11 个、
`.claude` 22 个、`.agents` 11 个),CLI 解析出 18 个,与 TUI 面板一致;
其中 `chatgpt-image-beautify` 的目录名是 `2` —— **技能名取自 frontmatter
而非目录名**,自己扫盘必踩这种坑。

- **列表**:搜索框 + 名称 + 完整说明(TUI 只显示一行截断)+ 状态标签 +
  来源(全局/本文件夹/内置)。同名副本折成 `×N` 标记,不列成多行;
- **键盘**:↑↓ 选择 · Enter 使用 · Esc 关闭(与 TUI 一致);
- **两种使用方式**:
  - 已登记的技能 → 点一下把「使用「X」技能:」写进输入框
    (opencode 靠 description 让模型自己挑,点名最稳);
  - 任何技能(含未加载的)→ `⤵` 直接把正文注入这一轮消息
    (`agent_send` 的 `skill_path` 参数,后端读文件组装
    `<skill_content name=…>` + 用户原话;界面与库里仍存原话);
- **诊断未加载的技能**(TUI 完全不提示):实测
  `argument-hint: [稿子] [男声|女声]` 是非法 YAML(流序列后又跟 token),
  **opencode 整份技能拒收**,而 Claude Code 容忍 —— 从 Claude 那边装来的
  技能极易悄悄失效(用户机器上正好有 3 个)。面板标「未加载」并给出原因;
- **制作/编辑/删除**:opencode 没有任何 skills 子命令,全靠手写目录 +
  YAML,所以表单价值很大。填「技能名 / 什么时候用它 / 技能指令 / 作用范围」
  即生成合规 `SKILL.md`;改写时**保留 name/description 之外的字段**并给会炸
  YAML 的值补引号 —— 坏技能在编辑器里保存一次就自动修好(实测
